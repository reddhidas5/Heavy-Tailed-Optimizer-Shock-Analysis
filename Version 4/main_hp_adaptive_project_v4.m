function main_hp_adaptive_project_v4()
% MAIN_HP_ADAPTIVE_PROJECT_V4
...
% -------------------------------------------------------------------------
    ...
    config.data_filename  = 'hp_adaptive_v4_results.mat';
    config.save_figures   = true;
    config.figure_prefix  = 'hp_adaptive_v4_';
    ...
    fprintf(' Trials: %d | Iterations: %d\n', ...
        config.num_trials, config.num_iters);
    fprintf('============================================================\n');

    results = run_experiments(config, problem);
    ...
    stats = compute_statistics(config, results);

    %% ========================= IMPULSE EXPERIMENT =========================
    if config.run_impulse_experiment
        fprintf('\n============================================================\n');
        fprintf(' Running controlled impulse-response experiment\n');
        fprintf('============================================================\n');

        impulse = run_impulse_experiment(config, problem);
    else
        impulse = [];
    end

    if config.run_paired_shock_study
        fprintf('\n============================================================\n');
        fprintf(' Running V4 paired-control causal shock study\n');
        fprintf('============================================================\n');
    ...
    plot_convergence_bands(config, stats);
    plot_violation_probabilities(config, stats);
    plot_risk_summary(config, stats);
    plot_clipping_rate(config, stats);
    plot_effective_step_scale(config, stats);

    for kk = config.tail_iters
        if kk <= config.num_iters
            plot_update_survival(config, results, kk);
            plot_error_survival(config, results, kk);
        end
    end

    plot_hill_final(config, stats);
    plot_hill_sensitivity(config, stats);

    if ~isempty(impulse)
        plot_impulse_response(config, impulse);
    end
    ...
    if config.export_csv
        export_summary_tables(config, stats);
    end

    if config.save_data
        save(config.data_filename, ...
            'config','problem','results','stats','tuned','impulse','shock','-v7.3');
...
function export_paired_shock_table(config,shock)
    ...
    writetable(Tpaired,'hp_adaptive_v4_paired_shock_summary.csv');

    fprintf('Exported:\n');
    fprintf('  hp_adaptive_v4_paired_shock_summary.csv\n');
end
